	...
	IN	PSTRING		  pInput)
{
	...
	{
		Value = simple_strtol(pValueStr, 0, 10);
		/* Force at BW_20 for HE */
		DBGPRINT(RT_DEBUG_ERROR, ("###### Force at HT20 (BW_20) mode !!! ########\n"));
		//Value = BW_20;
		if (Value == BW_40)
			pAd->CommonCfg.RegTransmitSetting.field.BW = BW_40;
		else
...
#ifdef DFS_SUPPORT
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			if(RTMPGetKeyParameter("P2P_NoAInv", tmpbuf, 10, pBuffer, TRUE))
			{
				...
				DBGPRINT(RT_DEBUG_ERROR, ("P2P_NoAInv=%lu\n", pAd->P2pCfg.GONoASchedule.Interval));
			}
		...
		if (RTMPGetKeyParameter("BW20_RSSI_THR0", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW20_RSSI_THR0_Proc(pAd, tmpbuf);	
		if (RTMPGetKeyParameter("BW20_RSSI_THR1", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW20_RSSI_THR1_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter("BW20_RSSI_THR2", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW20_RSSI_THR2_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter("BW20_RSSI_THR3", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW20_RSSI_THR3_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter("BW40_RSSI_THR0", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW40_RSSI_THR0_Proc(pAd, tmpbuf);	
		if (RTMPGetKeyParameter("BW40_RSSI_THR1", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW40_RSSI_THR1_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter("BW40_RSSI_THR2", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW40_RSSI_THR2_Proc(pAd, tmpbuf);
		if (RTMPGetKeyParameter("BW40_RSSI_THR3", tmpbuf, 10, pBuffer, TRUE))
		  Set_BW40_RSSI_THR3_Proc(pAd, tmpbuf);
