...
loadfw_protect:
	...
	//DataBuffer = kmalloc(14592, GFP_ATOMIC | GFP_DMA);
	//DataDMA= virt_to_phys(DataBuffer);
	DataBuffer = RTMPQMemAddr(14592, &DataDMA, BLK_TX4);
...
/* ++ dump firmware ++ */
#if 0
			FWIndex = pChipCap->FWImageName + 32 + CurLen;
			RAMIndex = CurLen;
#endif
/* ++ dump firmware ++ */
...
/* ++ dump firmware ++ */
			...
			for ( index = 0 ; index < DumpLen ; index++ )
			{
				...
				if ( (index % 4) == 3 )
				{
					if ( memcmp( dump, FWIndex, 16) != 0 )
					{
						printk("error!\n");
						hex_dump("FW in RAM", dump, 16);
						hex_dump("FW in BIN ", FWIndex, 16);
					}
					FWIndex += 16;
				}
			}
			...
			if ( DumpLen % 4 )
			{
				if ( memcmp( dump, FWIndex, ( DumpLen % 4 ) * 4) != 0 )
				{
					printk("error!\n");
					hex_dump("FW in RAM", dump, ( DumpLen % 4 ) * 4);
					hex_dump("FW in BIN ", FWIndex, ( DumpLen % 4 ) * 4);
				}
			}
#endif
/* ++ dump firmware ++ */
...
#ifdef CONFIG_PRE_ALLOC
	//kfree(DataBuffer);
	NdisZeroMemory(DataBuffer, 14592);
#else
...
INT USBKickOutCmd(PRTMP_ADAPTER pAd, UCHAR *Buf, UINT32 Len)
{
	...
	/* Allocate TransferBuffer */
#ifdef CONFIG_PRE_ALLOC
	DataBuffer = kmalloc(512, GFP_ATOMIC | GFP_DMA);
        DataDMA= virt_to_phys(DataBuffer);
#else
...
INT AsicSendCmdToAndes(PRTMP_ADAPTER pAd, struct CMD_UNIT *CmdUnit)
{
	...
	{
		/*there is a crash problem in ATC platform when making suspend->resume test with 2 pieces of 7601 dongle,
		the null pointer is "pAd->MCUCtrl.CmdRspEventList.Next",so we add a protection here*/
		if(!(pAd->MCUCtrl.CmdRspEventList.Next))
		{
			DBGPRINT(RT_DEBUG_ERROR, ("%s==> Error:Null pointer occur!\n", __FUNCTION__));
			Ret = NDIS_STATUS_FAILURE;
			goto error;
		}
...
static VOID PBFMonitorRspEventHandler(PRTMP_ADAPTER pAd, UCHAR *Data)
{
	...
	if ((*Data) == 0) {
		...
		/* High threshold happens */
		DBGPRINT(RT_DEBUG_TRACE, ("%s::disable TX Queue\n", __FUNCTION__));
