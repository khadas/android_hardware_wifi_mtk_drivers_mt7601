...
/*set Dynamic VGA as default setting when scanning, otherwise, we may missing AP beacon. Add by xiaojiao.li*/
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
						...
						ie_list, &LenVIE, pVIE))
	{
		...
		if (RTMPEqualMemory(ie_list->Ssid, "DIRECT-", 7))
			DBGPRINT(RT_DEBUG_TRACE, ("1. P2P SCANNING: %s [%lu]\n", ie_list->Ssid, Idx));

		RT_CFG80211_SCANNING_INFORM(pAd, Idx, ie_list->Channel, (UCHAR *)pFrame,
									Elem->MsgLen, Rssi);
#endif /* RT_CFG80211_SUPPORT */
#endif /* LINUX */
	}
...
#ifdef RT_CFG80211_SUPPORT
                	if (RTMPEqualMemory(ie_list->Ssid, "DIRECT-", 7))
                        	DBGPRINT(RT_DEBUG_TRACE, ("2. P2P SCANNING: %s [%lu]\n", ie_list->Ssid, Idx));

			RT_CFG80211_SCANNING_INFORM(pAd, Idx, ie_list->Channel, Elem->Msg,
										Elem->MsgLen, Rssi);
#endif /* RT_CFG80211_SUPPORT */
#endif /* LINUX */
		}
		...
		*/
		...
		{			
			...
			else
			{
				...
				pBssEntry = &pAd->MlmeAux.SsidBssTab.BssEntry[Bssidx];
